...
c***********************************************************************
      ...
      real q(jd,kd,nq), s(jd,kd,nq),turmu(jd,kd),tscale(jd,kd),bt(jd,kd)
...
!     c       +4.*bSq*cjkl*rr1*cjkl*rr1)
!          term = 0.5*((bSq+1.)*qqx+X)
!          Y = sqrt(((1.-bSq)*qqy)*((1.-bSq)*qqy)
!     c       +4.*bSq*cjkl*rr2*cjkl*rr2)
!
...
!        a(j,4) = qq*a2 + ge(j1,k)*a4 + chkx*qqx*s4
        a4 = chkx*gm1*(ri1*s2 + ri2*s3 + qq*s4)
        a2 = chkx*(s1 + uu*s2 + vv*s3 + ge(j,k)*s4)
        a(j,1) = -qq*a2  + uv2*a4 + chkx*qqx*s1
        a(j,2) =  ri1*a2 - uu*a4  + chkx*qqx*s2
        a(j,3) =  ri2*a2 - vv*a4  + chkx*qqx*s3
        a(j,4) =  a4              + chkx*qqx*s4

        do n=1,4
           tmp(n)=a(j,n)
        enddo
        
        a(j,1) =uv(j,k)*tmp(1)+uu*tmp(2)+vv*tmp(3)+ge(j,k)*tmp(4)
        a(j,2) =-uu*a(j,1)
        a(j,3) =-vv*a(j,1)
        a(j,4) = a(j,1)
...
!        b(j,4) = qq*a2 + ge(j,k1)*a4 + chky*qqy*s4
        a4 = chky*gm1*(ri1*s2 + ri2*s3 + qq*s4)
        a2 = chky*(s1 + uu*s2 + vv*s3 + ge(j,k)*s4)
        b(j,1) = -qq*a2  + uv2*a4 + chky*qqy*s1
        b(j,2) =  ri1*a2 - uu*a4  + chky*qqy*s2
        b(j,3) =  ri2*a2 - vv*a4  + chky*qqy*s3
        b(j,4) =  a4              + chky*qqy*s4

        do n=1,4
           tmp(n)=b(j,n)
        enddo
        
        b(j,1) =uv(j,k)*tmp(1)+uu*tmp(2)+vv*tmp(3)+ge(j,k)*tmp(4)
        b(j,2) =-uu*b(j,1)
        b(j,3) =-vv*b(j,1)
        b(j,4) = b(j,1)
...
!        b(j,2) = ri1*a2 + uu*a4      + chky*qqy*s2
!        b(j,3) = ri2*a2 + vv*a4      + chky*qqy*s3
!        b(j,4) = qq*a2 + ge(j,k)*a4 + chky*qqy*s4
        a4 = chky*gm1*(ri1*s2 + ri2*s3 + qq*s4)
        a2 = chky*(s1 + uu*s2 + vv*s3 + ge(j,k)*s4)
        b(j,1) = -qq*a2  + uv2*a4 + chky*qqy*s1
        b(j,2) =  ri1*a2 - uu*a4  + chky*qqy*s2
...
c***********************************************************************
      integer jd,kd,js,je,ks,ke
      real q(jd,kd,nq), s(jd,kd,nq),turmu(jd,kd),tscale(jd,kd)
      real ug(jd,kd), vg(jd,kd)
      real xx(jd,kd), xy(jd,kd), yx(jd,kd), yy(jd,kd)
...
!        a(j,4) = qq*a2  + ge(j,k)*a4 +spec*s4
        a4 = chkx*gm1*(ri1*s2 + ri2*s3 + qq*s4)
        a2 = chkx*(s1 + uu*s2 + vv*s3 + ge(j,k)*s4)
        a(j,1) = -qq*a2  + uv2*a4 + spec*s1
        a(j,2) =  ri1*a2 - uu*a4  + spec*s2
        a(j,3) =  ri2*a2 - vv*a4  + spec*s3
        a(j,4) =  a4              + spec*s4
  121 continue
...
!        b(j,4) = qq*a2  + ge(j,k)*a4 +spec*s4
        a4 = chky*gm1*(ri1*s2 + ri2*s3 + qq*s4)
        a2 = chky*(s1 + uu*s2 + vv*s3 + ge(j,k)*s4)
        b(j,1) = -qq*a2  + uv2*a4 + spec*s1
        b(j,2) =  ri1*a2 - uu*a4  + spec*s2
        b(j,3) =  ri2*a2 - vv*a4  + spec*s3
        b(j,4) =  a4              + spec*s4
  122 continue
...
c
         ...
         a1 = s(j,k,1) + s(j,k,2)*uu + s(j,k,3)*vv + s(j,k,4)*uv2
         a2 = rj2*s(j,k,2)-rj1*s(j,k,3) + (rj2*uu - rj1*vv)*s(j,k,4)
         a3 = a1 + s(j,k,4)*cjkl*cjkl/gm1
         a4 = rj1*s(j,k,2) + rj2*s(j,k,3) + qq1*s(j,k,4)/rr1
...
c multiply by N_inverse, s(j,k,1) is unchanged
      ...
      do 121 j = js-1,je+1
         ...
         a4 = a1*s(j,k,2) + 0.5*(s(j,k,3)-s(j,k,4))*a2/cjkl
         a5 =-a2*s(j,k,2)*cjkl+0.5*((a1+1)*s(j,k,3)-(a1-1)*s(j,k,4))
         a6 = a2*s(j,k,2)*cjkl+0.5*((a1+1)*s(j,k,4)-(a1-1)*s(j,k,3))
...
c...multiply by T_eta
      ...
      do 131 j = js-1,je+1
         ...
         a1 = s(j,k,1) - 0.5*(s(j,k,3) + s(j,k,4))

         a2 = -a1*uv(j,k)*gm1*c2i
         a2 = a2 + s(j,k,1) + s(j,k,2)*(vv*rk1 - uu*rk2)
         a2 = a2 - (s(j,k,3) - s(j,k,4))*(uu*rk1 + vv*rk2)*0.5/cjkl

         a3 = a1*uu*gm1*c2i
         a3 = rk2*s(j,k,2) + 0.5*rk1*(s(j,k,3)-s(j,k,4))/cjkl
 
         a4 = a1*vv*gm1*c2i
         a4 = -rk1*s(j,k,2) + 0.5*rk2*(s(j,k,3)-s(j,k,4))/cjkl
...
c
     ...
     c       +4.*bSq*cx(j,k)*cx(j,k))/rr1
         ...
         a2 = rj2*s(j,k,2)-rj1*s(j,k,3) + (rj2*uu - rj1*vv)*s(j,k,4)
         a3 = a1 + s(j,k,4)*cjkl*cjkl/gm1
         a4 = rj1*s(j,k,2) + rj2*s(j,k,3) + qqmxt*s(j,k,4)/rr1
...
c multiply by N_inverse, s(j,k,1) is unchanged
     ...
     c       +4.*bSq*cy(j,k)*cy(j,k))/rr2
         ...
         a4 = a1*s(j,k,2) + (s(j,k,3)-s(j,k,4))*a2*bSq/X1
         a5 =-a2*s(j,k,2)*Y/bSq+( (Y*a1-Z1)*s(j,k,3)-(Y*a1-Y1)*s(j,k,4))/X1
         a6 =-a2*s(j,k,2)*Z/bSq+(-(Z*a1-Y1)*s(j,k,4)+(Z*a1-Z1)*s(j,k,3))/X1
...
c...multiply by T_eta
     ...
     c       +4.*bSq*cy(j,k)*cy(j,k))/rr2
         ...
         a1 = s(j,k,1) + (Z*s(j,k,3) - Y*s(j,k,4))/X

         a2 = -a1*uv(j,k)*gm1*c2i
         a2 = a2 + s(j,k,1) + s(j,k,2)*(vv*rk1 - uu*rk2)
         a2 = a2 - (s(j,k,3) - s(j,k,4))*(uu*rk1 + vv*rk2)*bSq/X

         a3 = a1*uu*gm1*c2i
         a3 = rk2*s(j,k,2) + rk1*(s(j,k,3)-s(j,k,4))*bSq/X
 
         a4 = a1*vv*gm1*c2i
         a4 = -rk1*s(j,k,2) + rk2*(s(j,k,3)-s(j,k,4))*bSq/X
...
c**********************************************************************
      ...
      real eig1,eig2,eig3,epsval,eps,fac,visfac
...
c..some initialization
      ...
      fac     = 1.05
      !am visfac  = 1.5 !orig
      visfac  = 2.0 !1.8, older test value
...
c        euler terms
            b1(m) = b1(m) + diag_minus(m+1,i)
            d1(m) = d1(m) - diag_plus(m-1,i)
            c1(m) = c1(m) + diag_plus(m,i) - diag_minus(m,i)

c        viscous terms
            b1(m) = b1(m) - visfac*0.5*vistrm3(m+1)
            c1(m) = c1(m) + visfac*vistrm2(m)
            d1(m) = d1(m) - visfac*0.5*vistrm1(m-1)
