...
c***  first executable statement
      ...
      if(obj_samod .and.(mod(istep0,nrest).eq.0.or.istep0.eq.2)) then
         call store_coeff_prod(x, y, obj_coeff_prod, jd, kd)
      end if
...
c...compute vorticity & distance function
     ...
     &              ug,vg,aj,bj,cj,ak,bk,ck,jd,kd,jbeg,jend,kbeg,kend,nsp,im)
...
10    continue
      ...
      if(obj_samod .and.(mod(istep0,nrest).eq.0.or.istep0.eq.2)) then
         ...
         call store_sadestruction(dest_store, jd, kd,nsp,im)
      end if
...
c***********************************************************************
      ...
      ! local variables
      integer j,k
      character*60 :: int_str,int_str1,gfile
      ...
      if(nspec.le.1) then
        if(num_grids.eq.1) then
          gfile='production.dat'
        else
          write(int_str1,'(i7)')im
          gfile='production'//trim(adjustl(int_str1))//'.dat'
        endif
      else
        if(num_grids.eq.1) then
          ...
          write(int_str1,'(i7)')im
          write(int_str,'(i7)')nsp
          gfile='production'//trim(adjustl(int_str1))//'_TS'
     >                      //trim(adjustl(int_str))//'.dat'
      end if
      
      !am open(2211,file='production.dat',form='unformatted')
      open(2211,file=gfile,form='unformatted')
      write(2211) jd, kd
      write(2211) ((prod_store(j,k),j=1,jd),k=1,kd)
      close(2211)
      return
...
c***********************************************************************
      ...
      if(nspec.le.1) then
        if(num_grids.eq.1) then
          ...
          write(int_str1,'(i7)')im
          gfile='destruction'//trim(adjustl(int_str1))//'.dat'
        endif
      else
        if(num_grids.eq.1) then
          gfile='destruction_TS'//trim(adjustl(int_str))//'.dat'
          ...
          write(int_str1,'(i7)')im
          write(int_str,'(i7)')nsp
          gfile='destruction'//trim(adjustl(int_str1))//'_TS'
     >                       //trim(adjustl(int_str))//'.dat'
      end if
      ...
      open(2211,file=gfile,form='unformatted')
      write(2211) jd, kd
      write(2211) ((dest_store(j,k),j=1,jd),k=1,kd)
      close(2211)
      return

      end

      subroutine store_coeff_prod(x, y, coeff_prod, jd, kd)
      implicit none
      integer jd,kd,j,k
      real :: x(jd,kd), y(jd,kd), coeff_prod(jd,kd)
      
      open(99, file = 'coeffprod.plt', form = 'formatted')
      write(99, *) "TITLE = ""Coeff Prod"""
      write(99, *) "VARIABLES = ""X"" ""Y"" ""beta"""
      write(99,*) "ZONE"
      write(99,*) "I = ",jd,", J = ",kd
      write(99,*) "ZONETYPE = Ordered, DATAPACKING = BLOCK" 
      write(99,'(F22.13)') ((x(j,k),j=1,jd),k=1,kd)
      write(99,'(F22.13)') ((y(j,k),j=1,jd),k=1,kd)
      write(99,*) ((coeff_prod(j,k),j=1,jd),k=1,kd)
      close(99)
